...
`/plan` 会开启会话内写工具审批模式（旧称 plan-only）。该模式下读工具仍可执行，写工具会先产出等待审批事件；用户输入 `y` 批准当前写工具一次，输入 `n` 拒绝并把结构化拒绝结果回填给模型，输入 `c` 取消本轮。批准只放行当前工具一次，不会关闭该模式。

`/do` 关闭该模式，`/clear` 会清空会话历史并复位该模式。输入提示的底部状态栏会在 `[DEFAULT]` 和 `[PLAN]` 之间动态切换。
...
当决定为 `ASK` 时，终端使用中文展示工具、脱敏参数、风险原因、规则来源和当前档位。普通审批支持本次允许、本会话允许、当前项目永久允许、拒绝和取消；未声明安全授权参数或开启 `/plan` 模式时只提供本次允许、拒绝和取消。项目授权只有原子写入成功后才执行当前调用，失败时原文件和工具均保持不变。
