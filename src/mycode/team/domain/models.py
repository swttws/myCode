...
class TeamState(str, Enum):
    ...
    ACTIVE = "active"  # 团队可以接受批次和成员操作。
    ARCHIVED = "archived"  # 团队已归档，只允许读取。
    RECOVERY_REQUIRED = "recovery_required"  # 上次运行未正常收尾，需要恢复。


class MemberState(str, Enum):
    """团队成员 worker 的生命周期和执行状态。"""

    PROVISIONING = "provisioning"  # 正在准备工作区和执行后端。
    RUNNING = "running"  # 成员正在处理分配的任务。
    IDLE = "idle"  # 成员已启动但当前没有待处理消息。
    AWAITING_APPROVAL = "awaiting_approval"  # 等待计划或权限审批。
    AWAITING_INPUT = "awaiting_input"  # 等待 Lead 或用户回答问题。
    BLOCKED = "blocked"  # 成员暂时无法推进，等待 Lead 恢复。
    STOPPING = "stopping"  # 正在保存检查点并关闭。
    STOPPED = "stopped"  # 已正常停止，可被重新唤醒。
    FAILED = "failed"  # 发生不可恢复的运行错误。


class MemberBackend(str, Enum):
    """Lead 请求成员时可选择的执行后端。"""

    AUTO = "auto"  # 由服务端按能力和优先级选择。
    TMUX = "tmux"  # 使用 tmux 启动独立 worker。
    TERMINAL = "terminal"  # 使用 Windows Terminal 启动独立 worker。
    IN_PROCESS = "in_process"  # 在当前进程内运行 worker。


class ResolvedBackend(str, Enum):
    """服务端最终选定的成员执行后端。"""

    TMUX = "tmux"  # 已解析为 tmux 后端。
    WINDOWS_TERMINAL = "windows_terminal"  # 已解析为 Windows Terminal 后端。
    IN_PROCESS = "in_process"  # 已解析为当前进程后端。


class TaskKind(str, Enum):
    """任务允许的工作范围。"""

    CODE = "code"  # 允许成员修改代码并提交结果。
    READ_ONLY = "read_only"  # 只允许读取和分析，不修改工作区。


class TeamTaskState(str, Enum):
    """任务看板中的状态以及允许的推进阶段。"""

    PENDING = "pending"  # 已创建但尚未被成员领取。
    CLAIMED = "claimed"  # 已绑定成员，等待开始执行。
    AWAITING_APPROVAL = "awaiting_approval"  # 等待 Lead 审批成员计划。
    AWAITING_INPUT = "awaiting_input"  # 等待 Lead 或用户回答成员问题。
    RUNNING = "running"  # 成员正在执行任务。
    BLOCKED = "blocked"  # 任务被错误或依赖暂时阻塞。
    COMPLETED = "completed"  # 任务已完成并有结果。
    FAILED = "failed"  # 任务发生不可恢复错误。
    CANCELLED = "cancelled"  # 任务被明确取消。


class BatchState(str, Enum):
    """团队批次从启动到整合结束的状态。"""

    PENDING = "pending"  # 批次已创建但尚未开始执行。
    ACTIVE = "active"  # 批次中仍有待处理任务。
    BLOCKED = "blocked"  # 批次被成员问题或错误阻塞。
    INTEGRATING = "integrating"  # 所有任务完成，正在整合结果。
    COMPLETED = "completed"  # 批次已完成并生成整合结果。
    FAILED = "failed"  # 批次无法继续。
    CANCELLED = "cancelled"  # 批次被明确取消。


class ApprovalState(str, Enum):
    """任务计划或人工决策的审批结果。"""

    PENDING = "pending"  # 尚未做出审批决定。
    APPROVED = "approved"  # 已批准并可继续执行。
    REJECTED = "rejected"  # 已拒绝，调用方需要调整或停止。
    CANCELLED = "cancelled"  # 审批流程被取消。


class MessageProtocol(str, Enum):
    """成员与 Lead 之间的结构化消息类型。"""

    MESSAGE = "message"  # 普通定向协作消息。
    BROADCAST = "broadcast"  # 发给除发送者外所有成员的消息。
    PLAN_SUBMIT = "plan_submit"  # 成员提交任务计划供 Lead 审批。
    PLAN_DECISION = "plan_decision"  # Lead 返回任务计划审批结果。
    STATUS_UPDATE = "status_update"  # 成员或 Lead 发布状态变化。
    SHUTDOWN_REQUEST = "shutdown_request"  # Lead 请求成员保存并停止。
    SHUTDOWN_RESPONSE = "shutdown_response"  # 成员确认已保存并停止。
    TASK_ASSIGNMENT = "task_assignment"  # 服务端投递给新成员的初始任务。
    CLARIFICATION_REQUEST = "clarification_request"  # 成员请求 Lead 或用户判断。
    CLARIFICATION_RESPONSE = "clarification_response"  # Lead 返回成员问题的答案。
    TOOL_APPROVAL_REQUEST = "tool_approval_request"  # 成员请求工具动作审批。
    TOOL_APPROVAL_RESPONSE = "tool_approval_response"  # Lead 或用户返回工具审批结果。
    TASK_RESULT = "task_result"  # 成员提交结构化任务结果。


class EventRecipientType(str, Enum):
    """事件接收者的角色种类。"""

    LEAD = "lead"  # 团队 Lead 消费事件。
    MEMBER = "member"  # 团队 member 消费事件。


class TeamEventState(str, Enum):
    """持久化通信事件的处理状态。"""

    PENDING = "pending"  # 已写入，等待目标角色处理。
    PROCESSING = "processing"  # 目标角色正在处理。
    ACKED = "acked"  # 目标角色已经成功处理。
    FAILED = "failed"  # 达到重试上限后的终态失败。
...
class TeamEvent:
    ...
    sequence: int
    recipient_name: str
    recipient_type: EventRecipientType
    message: TeamMessage
    ...
    def __post_init__(self) -> None:
        ...
        _require_non_empty_string("recipient_name", self.recipient_name)
        _require_enum("recipient_type", self.recipient_type, EventRecipientType)
        _require_instance("message", self.message, TeamMessage)
        _require_enum("state", self.state, TeamEventState)
        ...
        _require_optional_utc_datetime("acknowledged_at", self.acknowledged_at)
        if self.state is TeamEventState.ACKED:
            if self.acknowledged_at is None:
                raise ValueError("acknowledged_at must be present when event is acked")
        elif self.acknowledged_at is not None:
            raise ValueError("acknowledged_at is only valid when event is acked")
        if self.state is TeamEventState.FAILED and self.attempts < 3:
            raise ValueError("failed events must have at least three attempts")


@dataclass(frozen=True)
class RoleEventCursor:
    """某一角色在事件序列中的确认进度。"""
    ...
    def __post_init__(self) -> None:
        _require_non_empty_string("recipient_name", self.recipient_name)
        _require_enum("recipient_type", self.recipient_type, EventRecipientType)
        _require_non_negative_int("acknowledged_sequence", self.acknowledged_sequence)
        _require_optional_utc_datetime("updated_at", self.updated_at)
...
class EventFailure:
    ...
    def __post_init__(self) -> None:
        ...
        if self.batch_id is not None:
            _require_non_empty_string("batch_id", self.batch_id)
        _require_positive_int("attempts", self.attempts)
        _require_non_empty_string("reason_code", self.reason_code)
        _require_non_empty_string("reason", self.reason)
        _require_enum("final_state", self.final_state, TeamEventState)
        if self.final_state is not TeamEventState.FAILED:
            raise ValueError("event failures must have final_state=FAILED")
        _require_utc_datetime("failed_at", self.failed_at)
