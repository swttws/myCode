...
logger = logging.getLogger("mycode.team.locking")


def _lock_context(**context: object) -> dict[str, object]:
...
class FileLease:
    ...
    async def acquire(cls, path: Path, *, config: TeamConfig, owner: str) -> "FileLease":
        ...
        logger.info(
            ...
            extra=_lock_context(path=path, action="acquire", phase="lock", owner=owner),
        )
        ...
        while True:
            ...
            try:
                write_task = asyncio.create_task(asyncio.to_thread(_write_lock, lease))
                try:
                    await asyncio.shield(write_task)
                except asyncio.CancelledError:
                    # The worker thread cannot be cancelled once it starts. Wait for
                    # it, then remove only the lock record created by this attempt.
                    try:
                        await asyncio.shield(write_task)
                    except FileExistsError:
                        pass
                    else:
                        await asyncio.to_thread(_remove_lock_if_owned, lease)
                    raise
                logger.info(
                    "team.lock.acquired",
                    extra=_lock_context(
    ...
    async def release(self) -> None:
        ...
        if record is None or record["token"] != self.token or record["owner"] != self.owner:
            logger.error(
                "team.lock.release.failed",
                extra=_lock_context(
                    path=self.path,
                    action="release",
                    phase="lock",
                    owner=self.owner,
                    reason_code="lock_not_owned",
                ),
            )
            raise TeamError(
                code="lock_not_owned",
...
def _remove_lock_if_owned(lease: FileLease) -> None:
    record = _read_lock_record(lease.path)
    if record is None or record["token"] != lease.token or record["owner"] != lease.owner:
        return
    try:
        lease.path.unlink()
...
def _maybe_reclaim_stale_lock(path: Path, config: TeamConfig) -> bool:
    ...
    if record is None:
        ...
        logger.info(
            ...
            extra=_lock_context(path=path, action="acquire", phase="lock", reason_code="missing_lock"),
        )
        return True
