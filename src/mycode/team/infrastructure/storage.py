...
class TeamStore:
    ...
    def event_log_path(self, team_name: str) -> Path:
        return self.team_root(team_name) / "events.jsonl"

    def event_cursors_path(self, team_name: str) -> Path:
        return self.team_root(team_name) / "event-cursors.json"

    def event_failures_path(self, team_name: str) -> Path:
        return self.team_root(team_name) / "event-failures.jsonl"
    ...
    def request_root(self, team_name: str) -> Path:
        return self.team_root(team_name) / "requests"

    def request_path(self, team_name: str, request_id: str) -> Path:
        root = self.request_root(team_name)
        return self._bounded_path(root, f"{_safe_segment(request_id)}.json")

    def ensure_writable(self, team_name: str) -> None:
