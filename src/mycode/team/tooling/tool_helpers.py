...
def batch_content(batch: Any) -> dict[str, Any]:
    return {"batch_id": batch.batch_id, "goal": batch.goal, "state": batch.state.value, "revision": batch.revision}
...
def error_result(tool_name: str, exc: Exception) -> ToolResult:
    ...
    reason = exc.code if isinstance(exc, TeamError) else ("invalid_argument" if isinstance(exc, ValueError) else "team_action_failed")
