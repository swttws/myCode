...
class _OrchestrationTool:
    ...
    async def execute_async(self, arguments, context=None):
        ...
        if missing:
            return error_result(self.tool_name, ValueError(f"缺少必填参数：{missing}"))
        try:
...
class TeamBatchStartTool(_OrchestrationTool):
    ...
    async def _execute(self, a):
        ...
        return success_result(
            self.tool_name,
            batch_content(await maybe_await(self._service.start_batch(goal))),
        )
...
class TeamMemberSpawnTool(_OrchestrationTool):
    ...
    description = "启动一个绑定任务和批次的团队成员。"
    properties = {
        "member_name": field("成员名称。"),
        "role_name": field("成员角色名称。"),
        "role_revision": field("角色版本号。", "integer"),
        "requested_backend": field(
            "请求的成员后端。", "string", enum=[x.value for x in MemberBackend]
        ),
        "task_id": field("成员负责的任务标识。"),
        "batch_id": field("所属批次标识。"),
        "goal": field("成员工作目标。"),
        "read_only": field("是否只读。", "boolean"),
        "approval_required": field("写入前是否需要审批。", "boolean"),
    }
    ...
    async def _execute(self, a):
        ...
        if isinstance(backend, Exception) or not isinstance(backend, MemberBackend):
            return backend
