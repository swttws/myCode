...
class _TaskTool:
    ...
    def _owner_check(self, arguments):
        if not self._member_name:
            return None
        task = _service_method(self._service, "get_task")(
...
class TeamTaskGetTool(_TaskTool):
    ...
    async def _execute(self, a):
        return success_result(
            ...
            task_content(
                await maybe_await(
                    _service_method(self._service, "get_task")(
    ...
    async def _execute(self, a):
        ...
        updated = await maybe_await(
            _service_method(self._service, "update_task")(
                _required(a, "task_id", self.tool_name), rev, patch
            )
        )
...
class TeamTaskClaimTool(_TaskTool):
    ...
    async def _execute(self, a):
        ...
        task = await maybe_await(
            _service_method(self._service, "claim_task")(
                _required(a, "task_id", self.tool_name), member, rev
            )
        )
    ...
    async def _execute(self, a):
        if self._member_name:
            ...
            if state == TeamTaskState.RUNNING.value:
                ...
                if (
                    callable(checker)
                    and await maybe_await(checker(self._member_name, current.task_id))
