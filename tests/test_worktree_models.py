...
def test_worktree_package_exports_public_lifecycle_classes_without_subagent_import():
    script = (
        ...
        "required = [\n"
        "    'WorktreeService', 'WorktreeCleaner', 'CleanupBatchResult',\n"
        "    'WorktreeDiagnostic', 'WorktreeDisposition', 'WorktreeDispositionResult',\n"
        "    'WorktreeError',\n"
        "]\n"
        "unexpected = ['Worktree' + 'Manager', 'ActiveWorkspace' + 'Registry']\n"
        "missing = [name for name in required if not hasattr(worktree, name)]\n"
        "present = [name for name in unexpected if hasattr(worktree, name)]\n"
        ...
        "if missing or present or duplicates or private or subagent_loaded:\n"
        "    print({'missing': missing, 'present': present, 'duplicates': duplicates, 'private': private, 'subagent': subagent_loaded})\n"
        "    raise SystemExit(1)\n"
    )
    env = os.environ.copy()
    env["PYTHONPATH"] = "src"
